...
export default function QueueListPage({
  ...
  const priorityCases = queueItems.filter(
    ...
    <div className="flex-1">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Real-time overview of your department queue.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        {/* Removed duplicate .map() for table headers. The correct header rendering is inside <thead> below. */}
        <div className="overflow-x-auto">
          <table className="w-full min-w-[840px] border-collapse">
            <thead>
              <tr className="border-b border-slate-100">
                {["TOKEN", "CUSTOMER", "CHECK-IN", "WAIT TIME", "STATUS"].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody>
              ...
              {(Array.isArray(filteredQueue) ? filteredQueue : []).map((p) => (
                ...
                >
                  <td className="px-3 py-3.5 text-sm font-semibold text-slate-900">
                    {p.ticket}
                  </td>
